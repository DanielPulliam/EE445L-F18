...
//   The objective is to start the ADC every 10 ms, so any time difference other than 10 ms is an error.
  for (timeDifferencesIndex = 0; timeDifferencesIndex < no_TimeDifferences; timeDifferencesIndex++) {
	...
	timeJitter1 = (timeJitter) * (float)12.5 * (float)pow(10,-9) * 1000 * 1000; // If this value is larger than 10 ms, this signifies an error
...
//   The x-axis has the ADC value and the y-axis is the number of occurrences of that ADC value.
  ...
  for (dataRecordingsIndex = 0; dataRecordingsIndex < no_DataRecordings; dataRecordingsIndex++) {
    ...
    if (currentADCDump[dataRecordingsIndex] < smallestADCValue) {
		...
		rangeADC = largestADCValue - smallestADCValue;
	...
	ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
	printf("Smallest ADC:\r%d\r\rLargest ADC:\r%d\r\rADC Range:\r%d", smallestADCValue, largestADCValue, rangeADC);
	DelayWait(75000000);
	ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
	printf("Min # Occurrences:\r%d\r\rMax # Occurrences:\r%d", min, max);
	DelayWait(75000000);
  ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
  printf("EE 445L Lab 2\rTime Jitter: %d us\r%d", timeJitter1, timeJitter);
  ST7735_PlotClear(0,max);
	
	if ((largestADCValue - smallestADCValue) >= 127) {
		for (j = smallestADCValue; j < largestADCValue; j++) {
			ST7735_PlotLine(no_occurrences[j]);
			ST7735_PlotNext();
		}
	}
	else {
		int midValue = ((largestADCValue + smallestADCValue) / 2);
		for (k = midValue - 64; k < midValue + 64; k++) {
			ST7735_PlotLine(no_occurrences[k]);
			ST7735_PlotNext();
		}
	}
...
int main(void){
  ...
  ST7735_OutString("EE 445L Lab 2\r\r");
	...
	ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
	ST7735_OutString("PHASE 1:\r\rBegin Sampling...");
...
// END OF DATA COLLECTION PHASE
	...
	ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
	printf("PHASE 1 Complete!\r\rTime running in...\rMain: 9.912 ms\r\rISR: 9.592 us");
	DelayWait(75000000);	
	ST7735_FillScreen(ST7735_BLACK); 
	ST7735_SetCursor(0,0);
	ST7735_OutString("PHASE 2:\r\rBegin Analyzing...");
	DelayWait(75000000);
